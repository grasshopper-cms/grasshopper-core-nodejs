module.exports = (function(){
    ...
    internal.buildKey = function(nodeid, filename) {
        return (internal.config.assetsDir ? internal.config.assetsDir + '/' : '') + nodeid + (filename ? '/' + filename : '');
    };
    ...
    internal.copyObject = function(sourceNodeId, sourceName, destNodeId, destName){
        ...
        this.s3.copyObject({
            Bucket: internal.config.bucket,
            ACL: 'public-read',
            CopySource: internal.buildKey(sourceNodeId, sourceName),
            Key: internal.buildKey(destNodeId, destName)
        }, function (err, data) {
            if(err){
                deferred.reject(err);
            }
    ...
    internal.deleteObject = function(nodeid, filename){
        ...
        if (archiveBucket) {
            internal.copyObject(nodeid, filename, path.join(archiveBucket, nodeid), filename)
                .fail(function(err){
                    deferred.reject(err);
                })
                .then(function(){
