...
let filterFactory = require('./search/filterFactory'),
    ...
    // Have to cache the models, since a model with the same name cannot be created twice by mongoose
    modelCache = {};

module.exports = function (connection, db) {

    let crud = require('./mixins/crud')(),
    ...
    content.model = collectionName => {
        ...
        if (modelCache[collectionName]) {
            ...
            let model = connection.model(collectionName, schema(collectionName));
    ...
    // need to customize create, since it depends on the contenttypes destination
    content.insert = function(obj, options) {
        let self = this,
            deferred = q.defer();

        db.contentTypes.model().findById(obj.meta.type)
            .then(contentType => {
                console.log('destination >>>', contentType.destination);

                this.model(contentType.destination).create(obj, function(err, doc){
                    if(err) {
                        deferred.reject(self.handleError(err));
                    } else {

                        self.getById(doc._id, options, contentType.destination)
