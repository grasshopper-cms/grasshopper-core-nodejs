...
var crud = require('./mixins/crud'),
    ...
    content = Object.create(crud, {
        model: {value: mongoose.model(collectionName, schema.create())},
        getModel : {value: getModel }
    }),
    modelCache = {};

function getModel(id) {
    var ContentType = mongoose.model('contentTypes');
    return ContentType
        .findById(id)
        .then(function(type) {
            var collection = type.destination || collectionName,
                model = modelCache[collection];

            if (!model) {
                model = mongoose.model(collection, schema.create(collection));
                modelCache[collection] = model;
            }
