module.exports = (function () {
    ...
    content.updateIds = function (changedIdsObject) {
        var deferred = q.defer(),
            fieldToUpdate = {};

        _.each(changedIdsObject.changes, function (change) {

            var findField = 'fields.' + change.find;

            fieldToUpdate[findField] = 'fields.' + change.change;

        });

        this.model.update({ 'meta.type': changedIdsObject.contentTypeId }, { $rename: fieldToUpdate })
            .exec(function (err) {
                if (err) {
                    deferred.reject(err);
                    return;
