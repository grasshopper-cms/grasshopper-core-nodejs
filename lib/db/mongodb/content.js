module.exports = (function () {
    ...
    function buildQuery (nodes, types, filters, options, content) {
        ...
        if (!!options.publishingWindow) {
            ...
            if (query[validFromKey]) {
                existingQuery = query[validFromKey];
                delete query[validFromKey];
                query.$and = query.$and || [];
                newQuery = {};
                newQuery[validFromKey] = existingQuery;
            ...
            if (query[validToKey]) {
                existingQuery = query[validToKey];
                delete query[validToKey];
                query.$and = query.$and || [];
                newQuery = {};
                newQuery[validToKey] = existingQuery;
                query.$and.push(newQuery);
                newQuery = {};
                newQuery[validToKey] = {
                    $gte: rightNow
