 ...
 * result: makes neccessary db calls and fs calls to delete node, subnodes, content and assets
 *
 * @param kontx
 * @param next
 */
module.exports = function deleteById(kontx, next){
    ...
    function deleteAssets(nodeid, cb){
        assets.deleteAll({nodeid: nodeid}).then(function(){
                cb();
            },
            function(){
                cb();
    ...
    function deleteAssetDirectories(nodeid, cb){
        assets.removeDirectory({nodeid: nodeid}).then(function(){
                cb();
            },
            function(){
                cb();
    ...
    async.waterfall([
        ...
        function(cb){
            ...
            db.content.query(nodeList, [], [], {}).then(function(contentList){
                async.each(contentList, deleteContent, function(){
                    cb();
                });
            },function(err){
                console.log(err);
