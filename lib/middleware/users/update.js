module.exports = function updateUser(kontx, next){
    ...
    function manageTokens(userObj){
        ...
        if(userObj.enabled === false){
            db.tokens.deleteByUserId(userObj._id.toString()).then(resolve).fail(handleError).done();
        }
        else {
            resolve();
        }
    ...
    function checkForRoleChange(existingUserObj){
        if(updatedUserObj.role !== existingUserObj.role){
            ...
            db.users.update(updatedUserObj).then(manageTokens).fail(handleError).done();
        }
    }

    if (!updatedUserObj.createdBy) {
        updatedUserObj.createdBy = {_id: kontx.user._id, displayName: kontx.user.displayName};
    }
    updatedUserObj.updatedBy = {_id: kontx.user._id, displayName: kontx.user.displayName};
    ...
    //User is updating themselves, validate that they are not changing their role since they are not an admin.
        ...
        db.users.getById(kontx.user._id).then(checkForRoleChange).fail(handleError).done();
    }
    else {
        next(createError(403, strings.group('errors').user_privileges_exceeded_role));
    }
