...
module.exports = function parse(kontx, next){
    ...
    if ( !_.isUndefined( user.identities.basic ) && !_.isNull( user.identities.basic ) ) {

        if ( !_.isUndefined( user.identities.basic.password ) && !_.isNull( user.identities.basic.password ) ) {
