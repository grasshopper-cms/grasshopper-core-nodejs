...
module.exports = function validate(kontx, next){
    ...
    function validateBasicIdentity(){
        ...
        if(kontx.route === 'users.insert') {
            ...
            } else if ( loginIsSet() && user.identities.basic.login.length < 4 ) {
                next( createError( 400, messages.login_too_short ) );
            } else if ( loginIsSet() ) {
                loginIsUnique( user._id )
                    .then(next)
                    .fail(function ( err ){
            ...
            if (_.has(user.identities.basic, 'login') && _.isEmpty(user.identities.basic.login)) { // user is submitting login and it is empty.
    ...
    function loginIsSet() {
        return !_.isEmpty( user.identities.basic.login );
    }
